...
colors_named = {'red': red, 'blue': blue, 'yellow': yellow, 'green': green, 'orange': orange, 'white': white, 'gray': gray}
X0=WIDTH/2
Y0=HEIGHT/2
A = 149.6*10**9
G = 6.57*10**(-11)
...
def write_space_objects_data_to_file(output_filename, space_objects):
    ...
    **space_objects** — список объектов планет и звёзд
    """
    with open('save_t.txt', 'w') as out_file:
...
for i in range(flag_obj):
	...
	vx= vx_planet[i]
	vy = vy_planet[i]
	color = colors_named[color_planet[i]]
	r = r_planet[i]
	m = m_planet[i]
